"""
Support InternVL2Model with transformers library
"""
import torch
...
class InternVL2Model(BaseModel):
    ...
    def build_transform(input_size):
        MEAN, STD = InternVL2Model.IMAGENET_MEAN, InternVL2Model.IMAGENET_STD
        transform = T.Compose([
    ...
    def dynamic_preprocess(image, min_num=1, max_num=12, image_size=448, use_thumbnail=False):
        ...
        target_aspect_ratio = InternVL2Model.find_closest_aspect_ratio(
    ...
    def load_image(image_file, input_size=448, max_num=12):
        ...
        transform = InternVL2Model.build_transform(input_size=input_size)
        images = InternVL2Model.dynamic_preprocess(image, image_size=input_size, use_thumbnail=True, max_num=max_num)
    ...
    def chatbot_to_messages(self, chatbot) -> list[str]:
        ...
        for _, (user_msg, model_msg) in enumerate(chatbot):
            if isinstance(user_msg, (tuple, list)):  # query is image path
                images.append(self.load_image(user_msg[0], max_num=12).to(torch.bfloat16).cuda())
            else:  # query is text
