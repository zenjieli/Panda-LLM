...
def get_cached_model_ids():
    ...
    gguf_dirs = [d for d in os.listdir(TRANSFORMERS_CACHE) if osp.isdir(osp.join(TRANSFORMERS_CACHE, d)) and d.lower().endswith("gguf")]

    # List all gguf files in TRANSFORMERS_CACHE
    gguf_files = [d for d in os.listdir(TRANSFORMERS_CACHE) if osp.isfile(osp.join(TRANSFORMERS_CACHE, d)) and d.lower().endswith(".gguf")]

    # Extract model IDs, like "org/model_name"
    model_ids = ["/".join(d.split("--")[1:]) for d in model_dirs]

    return model_ids + gguf_dirs + gguf_files
